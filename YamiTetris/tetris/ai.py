...
class Ai:
    ...
    def bumpinesses(self, heights):
        result = []
        for i in range(Var.board_start_y, len(heights)-1):
            ...
            for offset in range(Var.board_start_x, field.width):
                ...
                if not result is None:
                    ...
                    if workingPieceIndex == len(workingPieces)-1 :
                        ...
                        _, _, score = Ai.best(field, workingPieces, workingPieceIndex + 1, weights, Var.ai_best_fix_level)
    ...
    def choose(initialField, piece, next_piece, offsetX, weights, parent):
        field = Ai(len(initialField[0]), len(initialField))
        field.updateField(copy.deepcopy(initialField))

        offset, rotation, _ = Ai.best(field, [piece, next_piece], Var.ai_working_piece_index, weights,  Var.ai_choice_fix_level)
        moves = []
